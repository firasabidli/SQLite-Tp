...
class MainActivity : AppCompatActivity() {
    ...
    private fun getAllEtudiants(db: SQLiteDatabase): List<EtudiantBC> {
        ...
        val projection = arrayOf(
            ...
            EtudiantBC.EtudiantEntry.COLUMN_NAME_CLASSE
        )
        ...
        with(cursor) {
            while (moveToNext()) {
                ...
                val classe = getString(getColumnIndexOrThrow(EtudiantBC.EtudiantEntry.COLUMN_NAME_CLASSE))
                val etudiant = EtudiantBC(nom, prenom, phone, email, classe)
